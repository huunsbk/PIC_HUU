-- Round-scoped scoring rules: set mode, target score, and cap score per round.

CREATE OR REPLACE FUNCTION public.normalize_round_scoring_rules_v1(p_scoring_config jsonb)
RETURNS jsonb
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  ...
  v_default_max_score integer := COALESCE((p_scoring_config->>'maxScore')::integer, 15);
  v_default_cap_score integer := COALESCE((p_scoring_config->>'capScore')::integer, v_default_max_score);
  v_modes jsonb := COALESCE(p_scoring_config->'roundSetModes', '{}'::jsonb);
  v_rules jsonb := COALESCE(p_scoring_config->'roundScoringRules', '{}'::jsonb);
  v_key text;
  v_rule jsonb;
  v_mode text;
  v_max_score integer;
...
BEGIN
  ...
  END IF;
  IF v_default_max_score < 1 THEN
    v_default_max_score := 15;
  END IF;
  IF v_default_cap_score < v_default_max_score THEN
    v_default_cap_score := v_default_max_score;
  END IF;

  FOREACH v_key IN ARRAY ARRAY['group', 'r32', 'r16', 'r8', 'qf', 'sf', 'final']
  LOOP
    v_rule := COALESCE(v_rules->v_key, '{}'::jsonb);
    v_mode := COALESCE(v_rule->>'matchSetMode', v_modes->>v_key, v_default_mode);
    v_max_score := COALESCE((v_rule->>'maxScore')::integer, v_default_max_score);
    v_cap_score := COALESCE((v_rule->>'capScore')::integer, v_default_cap_score);

    IF v_mode NOT IN ('single', 'best_of_3') THEN
      RAISE EXCEPTION 'Invalid round set mode for %', v_key;
    END IF;
    IF v_max_score < 1 THEN
      RAISE EXCEPTION 'Invalid maxScore for %', v_key;
    END IF;
    IF v_cap_score < v_max_score THEN
      RAISE EXCEPTION 'Invalid capScore for %', v_key;
    END IF;
...
AS $$
DECLARE
  v_rules jsonb := public.normalize_round_scoring_rules_v1(COALESCE(p_scoring_config, '{}'::jsonb));
  v_key text;
  v_result jsonb := '{}'::jsonb;
BEGIN
  FOREACH v_key IN ARRAY ARRAY['group', 'r32', 'r16', 'r8', 'qf', 'sf', 'final']
  LOOP
    v_result := jsonb_set(v_result, ARRAY[v_key], to_jsonb(v_rules->v_key->>'matchSetMode'), true);
  END LOOP;
  RETURN v_result;
END;
$$;

CREATE OR REPLACE FUNCTION public.get_match_scoring_rule_v1(p_match_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO public, pg_temp
...
AS $$
BEGIN
  RETURN COALESCE(public.get_match_scoring_rule_v1(p_match_id)->>'matchSetMode', 'single');
END;
$$;
...
AS $$
DECLARE
  v_old_rules jsonb := public.normalize_round_scoring_rules_v1(COALESCE(p_old_scoring_config, '{}'::jsonb));
  v_new_rules jsonb := public.normalize_round_scoring_rules_v1(COALESCE(p_new_scoring_config, '{}'::jsonb));
  v_key text;
  v_label text;
...
BEGIN
  FOREACH v_key IN ARRAY ARRAY['group', 'r32', 'r16', 'r8', 'qf', 'sf', 'final']
  LOOP
    IF COALESCE(v_old_rules->v_key, '{}'::jsonb) = COALESCE(v_new_rules->v_key, '{}'::jsonb) THEN
      CONTINUE;
    END IF;
    ...
    IF v_has_result THEN
      v_label := CASE v_key
        ...
        ELSE v_key
      END;
      RAISE EXCEPTION 'ROUND_SCORING_RULE_LOCKED: % đã có kết quả. Hãy reset kết quả vòng này trước khi đổi số séc, điểm chạm đến hoặc điểm kịch trần.', v_label;
    END IF;
  END LOOP;
END;
$$;
...
BEGIN
  ...
  PERFORM public.normalize_round_scoring_rules_v1(COALESCE(p_scoring_config, '{}'::jsonb));
...
BEGIN
  ...
  v_rule := public.get_match_scoring_rule_v1(p_match_id);
  v_mode := COALESCE(v_rule->>'matchSetMode', 'single');
  ...
  v_max_score := COALESCE((v_rule->>'maxScore')::integer, (v_config->>'maxScore')::integer, 15);
  v_cap_score := COALESCE((v_rule->>'capScore')::integer, (v_config->>'capScore')::integer, v_max_score);
...
REVOKE ALL ON FUNCTION public.normalize_round_scoring_rules_v1(jsonb) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.normalize_round_set_modes_v1(jsonb) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_match_scoring_rule_v1(text) FROM PUBLIC, anon;
...
REVOKE ALL ON FUNCTION public.validate_event_config_v1(text, text, text, jsonb, jsonb) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.update_match_set_score_v1(text, integer, integer, integer) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.normalize_round_scoring_rules_v1(jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.normalize_round_set_modes_v1(jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_match_scoring_rule_v1(text) TO authenticated;
