-- Preserve seed-priority opening rounds without breaking round-robin invariants.
BEGIN;
...
BEGIN
  ...
  -- A NULL slot represents the one BYE required for an odd team count.
  ...
  -- Seed the Circle Method so round one is always 1-2, 3-4, 5-6...
  -- For odd groups the final real team is paired with the internal BYE.
  ...
  -- Reorder the valid Circle Method rounds by team one's opponent. This makes
  -- round two contain 1-3 while allowing the remaining pairs to stay valid.
  FOR v_output_round IN 1..(v_size - 1)
  LOOP
    IF v_output_round <= v_actual_count - 1 THEN
    ...
    -- Keep the requested seed order in round one. In later rounds, choose a
    -- boundary-safe first pair whenever possible to improve recovery time.
...
COMMENT ON FUNCTION public.build_round_robin_pairs_v1(text[]) IS
  'Builds complete seeded Circle Method rounds: adjacent seeds in round one and seed 1 versus seed 3 in round two.';
...
DECLARE
  ...
  v_invalid_opening_pairs integer;
  v_has_second_round_priority boolean;
BEGIN
  FOREACH v_team_count IN ARRAY ARRAY[3, 4, 5, 6, 7, 48, 64, 96]
  LOOP
    WITH pairs AS (
      SELECT *
    ...
    IF v_match_count <> (v_team_count * (v_team_count - 1)) / 2 THEN
      RAISE EXCEPTION 'Seeded round-robin self-test failed for % teams: % matches', v_team_count, v_match_count;
    END IF;

    IF v_round_count <> (CASE WHEN mod(v_team_count, 2) = 0 THEN v_team_count - 1 ELSE v_team_count END) THEN
      RAISE EXCEPTION 'Seeded round-robin self-test failed for % teams: % rounds', v_team_count, v_round_count;
    END IF;
    ...
    SELECT count(*)::integer
      INTO v_invalid_opening_pairs
    FROM pairs
    WHERE round_no = 1
      AND (
        team_a_id <> 'team-' || ((match_in_round * 2) - 1)::text
        OR team_b_id <> 'team-' || (match_in_round * 2)::text
      );

    IF v_invalid_opening_pairs > 0 THEN
      RAISE EXCEPTION 'Seeded round-robin opening order failed for % teams', v_team_count;
    END IF;
    ...
    ) INTO v_has_second_round_priority;

    IF NOT v_has_second_round_priority THEN
      RAISE EXCEPTION 'Seeded round-robin second-round priority failed for % teams', v_team_count;
    END IF;
    ...
    IF v_duplicate_pairs > 0 THEN
      RAISE EXCEPTION 'Seeded round-robin duplicate pairs for % teams', v_team_count;
    END IF;
    ...
    IF v_round_conflicts > 0 THEN
      RAISE EXCEPTION 'Seeded round-robin same-round conflicts for % teams', v_team_count;
    END IF;
  END LOOP;
END;
$$;
