-- Resolve locked manual KO slot sources to real teams for scoring/TV, while keeping the KO screen placeholder-only.
BEGIN;
...
BEGIN
  ...
  SET team_a_id = wm.team_a_id,
      ...
      metadata = jsonb_set(
        jsonb_set(COALESCE(m.metadata, '{}'::jsonb), '{resolved_team_id_a}', to_jsonb(wm.team_a_id), true),
        '{resolved_team_id_b}', to_jsonb(wm.team_b_id), true
      )
