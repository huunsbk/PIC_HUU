...
COMMENT ON FUNCTION public.setup_groups_v2(text, integer) IS
  'Commercial Beta V1: permission-checked group setup scoped to one event and the event real tenant_id; SUPER_ADMIN may operate across tenants.';

CREATE OR REPLACE FUNCTION public.dissolve_groups_v2(
  p_event_id text
)
RETURNS jsonb
LANGUAGE plpgsql
...
COMMENT ON FUNCTION public.dissolve_groups_v2(text) IS
  'Commercial Beta V1: permission-checked group dissolution scoped to one event and the event real tenant_id; SUPER_ADMIN may operate across tenants.';
...
REVOKE ALL ON FUNCTION public.dissolve_groups_v2(text) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.dissolve_groups_v2(text) TO authenticated;
