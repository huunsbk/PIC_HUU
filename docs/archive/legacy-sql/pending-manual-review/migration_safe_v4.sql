...
-- ==============================================================================
-- 1. AUTO EVENT ASSIGNMENT TRIGGER
-- Cấp quyền quản lý Event cho người tạo ra Event đó
-- ==============================================================================
...
-- ==============================================================================
-- 2. AUDIT LOG TRIGGER
-- Schema: id (bigint), timestamp (text), action (text), details (text), created_at (timestamptz), tenant_id (uuid)
-- ==============================================================================
CREATE OR REPLACE FUNCTION public.audit_matches_changes_safe_v4()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DROP TRIGGER IF EXISTS trg_audit_matches_safe_v4 ON public.matches;
CREATE TRIGGER trg_audit_matches_safe_v4
AFTER INSERT OR UPDATE OR DELETE ON public.matches
FOR EACH ROW EXECUTE FUNCTION public.audit_matches_changes_safe_v4();

-- ==============================================================================
-- 3. SOFT DELETE STRATEGY (ADD COLUMNS SAFELY)
-- ==============================================================================
...
-- ==============================================================================
-- 4. RLS POLICIES (TÁCH BIỆT CRUD ĐẠT CHUẨN EVENT SCOPE)
-- ==============================================================================
...
-- ==============================================================================
-- 5. ENTERPRISE INDEXES (HỖ TRỢ 1000+ CONCURRENT USERS)
-- ==============================================================================
