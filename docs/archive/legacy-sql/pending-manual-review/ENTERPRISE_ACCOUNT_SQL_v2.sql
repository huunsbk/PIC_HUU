-- ====================================================================================================
-- HỆ THỐNG ENTERPRISE ACCOUNTS SAAS MULTI-TENANT KHỞI LẬP (KHÔNG DOWNTIME) - VERSION 2
-- ====================================================================================================
...
-- Đã cập nhật (v2): Sửa lỗi cú pháp cấp quyền ON DELETE, xử lý Idempotent (Idempotency) để script có thể chạy nhiều lần không lỗi.
-- ====================================================================================================
...
CREATE TABLE IF NOT EXISTS public.accounts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL, -- Liên kết ẩn với Supabase Auth, đã sửa cú pháp SET NULL
...
CREATE INDEX IF NOT EXISTS idx_accounts_tenant_id ON public.accounts(tenant_id) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_acct_perms_account ON public.account_permissions(account_id);
...
CREATE INDEX IF NOT EXISTS idx_acct_event_perms_account ON public.account_event_permissions(account_id) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_login_logs_account ON public.login_logs(account_id);
CREATE INDEX IF NOT EXISTS idx_login_logs_created_at ON public.login_logs(created_at);

-- ====================================================================================================
-- 3. SEED DỮ LIỆU MẪU (SCAFFOLDING DATA) - CHẠY NHIỀU LẦN AN TOÀN (IDEMPOTENT)
-- ====================================================================================================
