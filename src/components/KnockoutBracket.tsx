...
export default function KnockoutBracket() {
  ...
  const handleGenerateBracket = async () => {
    ...
    if (matches.filter((m) => m.groupId === 'knockout').length > 0) {
      setErrorMessage('Đã có sơ đồ, hãy xóa trước khi tạo lại');
      return;
    }
    try {
      await generate_knockout_bracket_v1.mutateAsync(selectedEventId);
      setSuccessMessage('Đã tạo sơ đồ knockout.');
      setTimeout(() => setSuccessMessage(null), 3500);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Không tạo được sơ đồ knockout.';
      setErrorMessage(message.includes('already exists') ? 'Đã có sơ đồ, hãy xóa trước khi tạo lại' : message);
    }
  };
        ...
        {canManage && (
          <div className="flex flex-wrap items-center gap-3">
            ...
            {!isEditMode && (
              ...
              >
                <PlayCircle size={16} /> Tạo sơ đồ knockout
              </button>
            )}
